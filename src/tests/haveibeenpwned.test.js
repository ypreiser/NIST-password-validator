...
describe("HaveIBeenPwnedService", () => {
  ...
  describe("checkPassword", () => {
    ...
    it("should throw an error if API responds with a non-ok status", async () => {
      global.fetch = jest.fn(() =>
        Promise.resolve({
          ok: false,
        })
      ...
      await expect(
        HaveIBeenPwnedService.checkPassword("password123")
      ).rejects.toThrow("Failed to check password against HaveIBeenPwned API.");
    });
    it("should correctly parse the count of a matched hash", async () => {
      global.fetch = jest.fn(() =>
        Promise.resolve({
          ok: true,
          text: () =>
            Promise.resolve(
              "CBFDAC6008F9CAB4083784CBD1874F76618D2A97:42\n1234567890ABCDEF1234567890ABCDEF12345678:10\n"
            ),
        })
      );

      const count = await HaveIBeenPwnedService.checkPassword("password123");
      expect(count).toBe(42); // Correctly parsed count from matched hash
    });
    ...
    it("should return the count for a breached password", async () => {
      global.fetch = jest.fn(() =>
        Promise.resolve({
          ok: true,
          text: () =>
            Promise.resolve("CBFDAC6008F9CAB4083784CBD1874F76618D2A97:5\n"),
        })
      );
    ...
    it("should throw an error if fetch fails", async () => {
      ...
      await expect(
        HaveIBeenPwnedService.checkPassword("password123")
      ).rejects.toThrow("HaveIBeenPwned check failed: Network error");
    });
  ...
  describe("generateSHA1", () => {
    ...
    it("should generate unique hashes for different passwords", async () => {
      ...
      const hash2 = await HaveIBeenPwnedService.generateSHA1(
        "differentPassword"
      );
